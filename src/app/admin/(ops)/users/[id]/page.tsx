    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-xl font-extrabold text-white">{String(u.full_name ?? "")}</h1>
          <p className="text-sm text-zinc-400">{String(u.email ?? "") || "이메일 미동기 — 로그인 시 갱신됩니다"}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">계정·플랜</h2>
            <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
              <div>
                <dt className="text-xs text-zinc-500">플랜</dt>
                <dd className="font-semibold text-white">{formatPlanLabel(String(u.plan ?? ""))}</dd>
              </div>
              <div>
                <dt className="text-xs text-zinc-500">구독 상태</dt>
                <dd className="text-zinc-200">{String(u.subscription_status ?? "—")}</dd>
              </div>
              <div>
              ...
              <div>
                ...
                <dd className="text-zinc-200">{u.cancel_at_period_end ? "예" : "아니오"}</dd>
              </div>
              <div>
          ...
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">사용량 (이번 usage_month)</h2>
            ...
            <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-3">
              <div>
                ...
                <dd className="font-mono text-white">{String(u.usage_month ?? "—")}</dd>
              </div>
              <div>
                <dt className="text-xs text-zinc-500">AI 호출</dt>
                <dd className="tabular-nums text-white">{String(u.ai_calls_this_month ?? 0)}</dd>
              </div>
              <div>
                <dt className="text-xs text-zinc-500">문서 발송</dt>
                <dd className="tabular-nums text-white">{String(u.document_sends_this_month ?? 0)}</dd>
              </div>
            </dl>
          </section>

          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">공개 폼·포털</h2>
            ...
            <p className="mt-1 text-sm text-zinc-300">
              포털 고객(토큰 보유): <strong className="text-white">{detail.portalCustomerCount}</strong> / 전체 고객{" "}
              {detail.customersTotal}
            </p>
          </section>

          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">최근 문의·견적·청구</h2>
          ...
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">고객센터 티켓</h2>
          ...
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">빌링 이벤트</h2>
            <ul className="mt-2 max-h-60 space-y-1 overflow-y-auto text-xs text-zinc-400">
              {detail.billingEvents.map((e) => (
                <li key={e.id}>
                  <span className="text-zinc-500">{new Date(e.created_at).toLocaleString("ko-KR")}</span> ·{" "}
                  <span className="text-zinc-300">{e.kind}</span> — {e.message}
                </li>
              ))}
          ...
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">알림톡·메시징 실패</h2>
          ...
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">세금계산서 이슈</h2>
          ...
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">활동 로그</h2>
          ...
          <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
            <h2 className="text-sm font-bold text-zinc-300">운영 메모 이력</h2>
            <ul className="mt-2 space-y-2 text-sm text-zinc-300">
