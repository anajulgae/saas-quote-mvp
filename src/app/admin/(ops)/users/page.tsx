...
export default async function AdminUsersPage({
  ...
  if (!supabase) {
    return <p className="text-sm text-red-400">DB 연결 실패</p>
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-extrabold text-white">사용자</h1>
        <p className="mt-1 text-sm text-zinc-400">이름·이메일·플랜·구독·사용량·최근 활동을 한 번에 추적합니다.</p>
      </div>
      ...
      <form className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-end" method="get">
        <div className="flex flex-1 flex-col gap-1">
          ...
          <input
            ...
            className="rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white placeholder:text-zinc-600"
          />
        </div>
      ...
      {error ? <p className="text-sm text-red-400">목록 오류: {error}</p> : null}

      <div className="overflow-x-auto rounded-lg border border-zinc-800">
        <Table>
          <TableHeader>
            <TableRow className="border-zinc-800 hover:bg-transparent">
              <TableHead className="text-zinc-400">이메일 / 이름</TableHead>
              <TableHead className="text-zinc-400">플랜</TableHead>
              <TableHead className="text-zinc-400">구독</TableHead>
              <TableHead className="text-zinc-400">체험 종료</TableHead>
              <TableHead className="text-right text-zinc-400">AI</TableHead>
              <TableHead className="text-right text-zinc-400">발송</TableHead>
              <TableHead className="text-zinc-400">상태</TableHead>
              <TableHead className="text-zinc-400">가입</TableHead>
          ...
          <TableBody>
            {rows.length === 0 ? (
              ...
              rows.map((u) => (
                <TableRow key={u.id} className="border-zinc-800 text-zinc-200">
                  <TableCell>
                  ...
                  <TableCell className="text-xs">
                    {u.account_disabled ? (
                      ...
                      <span className="text-amber-400">해지예약</span>
                    ) : (
