...
export default async function AdminUsagePage() {
  ...
  return (
    <div className="space-y-10">
      <div>
        <h1 className="text-xl font-extrabold text-white">사용량·리소스</h1>
        <p className="mt-1 text-sm text-zinc-400">
          AI·문서 발송·공개 폼·포털 활성 규모를 봅니다. 월별 합계는 `users.usage_month` 가 현재 월인 행만 합산합니다.
        </p>
      </div>

      {u.error ? <p className="text-sm text-amber-300">데이터 경고: {u.error}</p> : null}

      <section className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4">
          <p className="text-xs font-bold uppercase text-zinc-500">집계 월</p>
          <p className="mt-1 text-2xl font-extrabold text-white">{u.month}</p>
        </div>
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4">
          <p className="text-xs font-bold uppercase text-zinc-500">공개 문의 폼 ON</p>
          <p className="mt-1 text-2xl font-extrabold text-white">{u.publicFormsEnabled}</p>
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4">
          ...
          <p className="mt-1 text-2xl font-extrabold text-white">{u.customersWithPortalApprox}</p>
          <p className="mt-1 text-xs text-zinc-500">샘플 최대 5000명 기준 고유 user_id 수</p>
        </div>
      </section>

      <section>
        <h2 className="mb-2 text-sm font-bold text-zinc-300">플랜별 이번 달 합계</h2>
        <div className="overflow-x-auto rounded-lg border border-zinc-800">
          <Table>
            <TableHeader>
              <TableRow className="border-zinc-800 hover:bg-transparent">
                <TableHead className="text-zinc-400">플랜</TableHead>
                <TableHead className="text-right text-zinc-400">사용자 수</TableHead>
                <TableHead className="text-right text-zinc-400">AI 합</TableHead>
                <TableHead className="text-right text-zinc-400">발송 합</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {Object.entries(u.byPlan).map(([plan, agg]) => (
                <TableRow key={plan} className="border-zinc-800 text-sm text-zinc-200">
      ...
      <section>
        <h2 className="mb-2 text-sm font-bold text-zinc-300">AI 호출 상위</h2>
        <RankTable rows={u.topAi} kind="ai" />
      </section>

      <section>
        <h2 className="mb-2 text-sm font-bold text-zinc-300">문서 발송 상위</h2>
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-zinc-800 hover:bg-transparent">
            ...
            <TableHead className="text-zinc-400">플랜</TableHead>
            <TableHead className="text-right text-zinc-400">{kind === "ai" ? "AI" : "발송"}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {rows.length === 0 ? (
            ...
            rows.map((r) => (
              <TableRow key={r.id} className="border-zinc-800 text-sm text-zinc-200">
                <TableCell>
