...
export function DashboardOperationalHub({
  ...
  const formUrl =
    hub.publicInquiryFormEnabled && hub.publicInquiryFormToken
      ? `${origin}/request/${encodeURIComponent(hub.publicInquiryFormToken)}`
      : null
    ...
    <section className="space-y-3" aria-labelledby="dash-hub-heading">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-[13px]">
        ...
        <Link
          href="/settings"
          className={cn(
            buttonVariants({ variant:"outline", size:"sm" }),"h-8 shrink-0 gap-1.5 text-xs font-medium"
          )}
        >
      ...
      <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
        <Card className={hubCardClass()}>
          ...
          <CardContent className="space-y-2 pt-0">
            {formUrl ? (
              <>
                <p className="truncate rounded-md border border-border/60 bg-muted/20 px-2 py-1.5 font-mono text-xs text-muted-foreground">
                  {formUrl}
                </p>
        ...
        <Card className={hubCardClass()}>
          ...
          <CardContent className="flex flex-wrap gap-1.5 pt-0">
            {portal ? (
              ...
              <span className="text-xs text-muted-foreground">Pro · 미니 포털</span>
            )}
            {proLanding ? (
              <Link
                ...
                {notificationPreview.map((n) => (
                  <li key={n.id}>
                    {n.linkPath?.startsWith("/") ? (
                      <Link
                        href={n.linkPath}
                        className={cn("block rounded-md border border-transparent px-1.5 py-1 text-xs leading-snug transition-colors hover:border-border/60 hover:bg-muted/30",
                      ...
                      <span className="block px-1.5 py-1 text-xs text-muted-foreground line-clamp-2">{n.title}</span>
                    )}
                  </li>
                ))}
              </ul>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-dashed border-border/55 bg-muted/10 px-3 py-2 text-xs text-muted-foreground">
